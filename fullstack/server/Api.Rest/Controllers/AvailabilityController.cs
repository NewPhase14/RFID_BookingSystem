...
[ApiController]
public class AvailabilityController(IAvailabilityService availabilityService, ISecurityService securityService)
    : ControllerBase
{
    ...
    public async Task<ActionResult<AvailabilityResponseDto>> CreateAvailability(
        [FromBody] AvailabilityCreateRequestDto dto, [FromHeader] string authorization)
    {
    ...
    public async Task<ActionResult<List<AvailabilityResponseDto>>> CreateAllAvailabilities(
        [FromBody] List<AvailabilityCreateRequestDto> dtos, [FromHeader] string authorization)
    {
        var jwt = securityService.VerifyJwtOrThrow(authorization);
        if (jwt.Role != "Admin") return Unauthorized();
    ...
    public async Task<ActionResult<AvailabilityResponseDto>> UpdateAvailability(
        [FromBody] AvailabilityUpdateRequestDto dto, [FromHeader] string authorization)
    {
        var jwt = securityService.VerifyJwtOrThrow(authorization);
        if (jwt.Role != "Admin") return Unauthorized();
    ...
    {
        var jwt = securityService.VerifyJwtOrThrow(authorization);
        if (jwt.Role != "User") return Unauthorized();
